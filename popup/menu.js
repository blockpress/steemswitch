...
document.addEventListener("click", (e) => {
	if (e.target.classList.contains("steemswitch")) {
		...
		gettingActiveTab.then((tabs) => {
			...
			if (destinationName=="Chainbb"){
				newUrl = newUrl.replace('/trending/','/topic/');
				newUrl = newUrl.replace('/created/','/topic/');
				newUrl = newUrl.replace('/hot/','/topic/');
				newUrl = newUrl.replace('/promoted/','/topic/');
			} else {
				newUrl = newUrl.replace('/topic/','/created/');
			}
